...
export function substituirInputsPorTexto(clone) {
  clone.querySelectorAll("input").forEach(input => {
    ...
    if (input.type === "checkbox") {
      // Renderiza checkbox como símbolo
...
/**
 * Substitui textareas por divs com valores formatados
 * @param {HTMLElement} clone - Clone do container
 */
export function substituirTextareasPorTexto(clone) {
  clone.querySelectorAll("textarea").forEach(textarea => {
    const div = document.createElement("div")
    div.style.border = "1px solid #999"
    div.style.padding = "8px"
    div.style.minHeight = (textarea.offsetHeight || 60) + "px"
...
export function prepararCloneParaPDF(elemento) {
  ...
  // Substitui textareas por texto
  substituirTextareasPorTexto(clone)
...
export function obterConfigPDF() {
  return {
    ...
    pagebreak: {
      ...
      avoid: [".comportamento", ".secao", "table", ".resultado-final", ".assinaturas"]
    }
  }
}
