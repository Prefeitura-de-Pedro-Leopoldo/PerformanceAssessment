...
export function calcularComportamento(compId) {
  ...
  if (todasNotasPreenchidas) {
    // Comportamentos 2 e 9 têm pontuação máxima diferente
    if (compId === 2 || compId === 9) {
      // Comportamento 2: máximo 15 pontos (3 critérios peso 1)
      // Comportamento 9: máximo 15 pontos (3 critérios: peso 2, 1, 2)
      const somaPesos = calcularSomaPesos(comportamento)
      pontuacaoNormalizada = (totalPontos / (somaPesos * 5)) * comportamento.max_pontos
    } else {
      // Outros comportamentos normalizam para seus pontos máximos
      const somaPesos = calcularSomaPesos(comportamento)
      pontuacaoNormalizada = (totalPontos / (somaPesos * 5)) * comportamento.max_pontos
    }
  }
