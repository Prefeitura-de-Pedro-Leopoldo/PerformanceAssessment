  ...
  comportamentos.forEach(comp => {
    ...
    html += `
                        <div class="total-comportamento">
                            <span>Total do Comportamento (Máximo: ${comp.max_pontos} pontos)</span>
...
function calcularComportamento(compId) {
  ...
  if (todasPreenchidas) {
    ...
    if (compId === 4 || compId === 9) {
      // Comportamentos 4 e 9: máximo 15 pontos
      totalNormalizado = (total / 15) * 15
    } else {
      // Outros comportamentos: máximo 10 pontos
...
function baixarPDF() {
  ...
  setTimeout(() => {
    ...
    const opt = {
      ...
      filename: "avaliacao_desempenho(Efetivo).pdf",
    ...
    clone.querySelectorAll("input").forEach(input => {
      ...
      if (input.type === "checkbox") {
        ...
        replacement.style.fontSize = "14pt"
